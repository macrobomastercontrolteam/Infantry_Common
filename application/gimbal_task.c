...
// #if GIMBAL_TEST_MODE
// //j-scope 帮助pid调参
// static void jscope_gimbal_test(void);
// #endif
...
// {
//     //电机数据指针获取
//     init->gimbal_yaw_motor.gimbal_motor_measure = get_motor_measure_ptr_by_CAN_ID(CAN_YAW_MOTOR_FEEDBACK_ID);
//     init->gimbal_pitch_motor.gimbal_motor_measure = get_motor_measure_ptr_by_CAN_ID(CAN_PIT_MOTOR_FEEDBACK_ID);
//     //陀螺仪数据指针获取
...
//         return 1;
//     }
//     else if ((ABS(gimbal_control.gimbal_yaw_motor.gimbal_motor_measure->given_current) >= YAW_MOTOR_CURRENT_LIMIT) || (ABS(gimbal_control.gimbal_pitch_motor.gimbal_motor_measure->given_current) >= PITCH_MOTOR_CURRENT_LIMIT))
//     {
//         fFatalError = 1;
//         return 1;
//     }
...
//         fSentryDbusEnable |= switch_is_down(chassis_move.chassis_RC->rc.s[CHASSIS_MODE_CHANNEL]) && switch_is_down(chassis_move.chassis_RC->rc.s[LEFT_LEVER_CHANNEL]);
