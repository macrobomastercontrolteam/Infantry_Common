...
//   */
// /**
//   * @brief          ????????????????????'gimbal_behaviour_mode_set'?????§Ú??
...
// /**
//   * @brief          ??????????????????????????????????????????
//   * @param[out]     gimbal_feedback_update:"gimbal_control"???????.
//   * @retval         none
//   */
...
// /**
//   * @brief          ??????????§»?????????????????yaw????Ú…????????yaw???
//   * @param[out]     mode_change:"gimbal_control"???????.
//   * @retval         none
//   */
...
//   */
// /**
//   * @brief          ????????????Ú…?????????????gimbal_behaviour_control_set?????????
//   * @param[out]     gimbal_set_control:"gimbal_control"???????.
//   * @retval         none
//   */
...
// /**
//   * @brief          ?????????????????Ú…???????????????????§á???
//   * @param[out]     gimbal_control_loop:"gimbal_control"???????.
//   * @retval         none
//   */
...
//   */
// /**
//   * @brief          ???PID????????pid??out,iout
//   * @param[out]     pid_clear:"gimbal_control"???????.
//   * @retval         none
//   */
...
// /**
//   * @brief          ???§µ?????
//   * @param[in]      gimbal_cali: §µ?????
//   * @param[out]     yaw_offset:yaw?????????
//   * @param[out]     pitch_offset:pitch ?????????
//   * @param[out]     max_yaw:yaw ???????§Ö???
//   * @param[out]     min_yaw: yaw ?????§³??§Ö???
//   * @param[out]     max_pitch: pitch ???????§Ö???
//   * @param[out]     min_pitch: pitch ?????§³??§Ö???
//   * @retval         none
//   */
...
//         gimbal_mode_change_control_transit(&gimbal_control); //???????§Ý? ???????????
//         gimbal_feedback_update(&gimbal_control);             //??????????
//         gimbal_set_control(&gimbal_control);                 //?????????????
//         gimbal_control_loop(&gimbal_control);                //???????PID????
...
// /**
//   * @brief          ???§µ????????§µ??????????????§³????§Ö?????
//   * @param[in]      yaw_offse:yaw ???
//   * @param[in]      pitch_offset:pitch ???
//   * @param[in]      max_yaw:max_yaw:yaw ????????
//   * @param[in]      min_yaw:yaw ??§³?????
//   * @param[in]      max_yaw:pitch ????????
//   * @param[in]      min_yaw:pitch ??§³?????
//   * @retval         ?????
//   * @waring         ???????????gimbal_control ????????????????????????????????
//   */
...
//                                                         - arm_sin_f32(feedback_update->gimbal_pitch_motor.relative_angle) * (*(feedback_update->gimbal_INT_gyro_point + INS_GYRO_X_ADDRESS_OFFSET));
    ...
    fill_buffer(yaw_angle, &(feedback_update->yaw_angle));
    fill_buffer(pitch_angle, &(feedback_update->pitch_angle));
}
...
// /**
//   * @brief          ??????????§»?????????????????yaw????Ú…????????yaw???
//   * @param[out]     gimbal_mode_change:"gimbal_control"???????.
//   * @retval         none
//   */
...
// /**
//   * @brief          ???PID????????pid??out,iout
//   * @param[out]     gimbal_pid_clear:"gimbal_control"???????.
//   * @retval         none
//   */
