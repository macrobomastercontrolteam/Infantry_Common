...
// VT13 shares USART6 with the referee/custom-controller stream; forward the
// received bytes into the referee FIFO so referee frames are still parsed.
...
void USART6_IRQHandler(void)
{
    ...
    else if(USART6->SR & UART_FLAG_IDLE)
    {
        ...
        if ((hdma_usart6_rx.Instance->CR & DMA_SxCR_CT) == RESET)
        {
            ...
            // The referee/custom-controller stream shares this USART6 link;
            // forward the raw bytes so referee frames (e.g. 0x0302) are parsed.
            referee_push_bytes(sbus_rx_buf[0], this_time_rx_len);
        }
        else
        {
            ...
            // The referee/custom-controller stream shares this USART6 link;
            // forward the raw bytes so referee frames (e.g. 0x0302) are parsed.
            referee_push_bytes(sbus_rx_buf[1], this_time_rx_len);
#endif
        }
    }
