...
static void shoot_feedback_update(void)
{
	...
	// For sentry upper head, receive it from lower head
...
bool_t isOverheated(void)
{
	...
	if ((toe_is_error(LOWER_HEAD_TOE) == 0) && (shoot_control.heat + SHOOT_HEAT_REMAIN_VALUE > shoot_control.heat_limit))
	{
		return 1;
	}
	else
