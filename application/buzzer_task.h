#ifndef _BUZZER_TASK_H
#define _BUZZER_TASK_H

#include "AHRS_middleware.h"
#include "chassis_behaviour.h"
#include "chassis_power_control.h"
#include "cmsis_os.h"
#include "gimbal_behaviour.h"
#include "gimbal_task.h"
#include "global_inc.h"
#include "referee.h"
#include "shoot.h"
#include <stdint.h>
#include <stdio.h>
#include <string.h>
#include "voltage_task.h"

#define BATTERY_TIMEOUT 120000
#define TOE_TIMEOUT 100000

#define Note_C3 (130) // 261.63Hz, 3822us
#define Note_D3 (146) // 293.66Hz, 3405us
#define Note_E3 (164) // 329.63Hz, 3034us
#define Note_F3 (174) // 349.23Hz, 2863us
#define Note_G3 (196) // 392.00Hz, 2551us
#define Note_A3 (220) // 440.00Hz, 2272us
#define Note_B3 (247) // 493.88Hz, 2052us

#define Note_C4 (262) // 261.63Hz, 3822us
#define Note_C4S (277) // 277.18Hz
#define Note_D4 (294) // 293.66Hz, 3405us
#define Note_E4 (330) // 329.63Hz, 3034us
#define Note_F4 (349) // 349.23Hz, 2863us
#define Note_G4 (392) // 392.00Hz, 2551us
#define Note_A4 (440) // 440.00Hz, 2272us
#define Note_B4 (494) // 493.88Hz, 2052us

#define Note_C5 (523) // 523.25Hz, 1911us
#define Note_C5S (554.37) 
#define Note_D5 (587) // 587.33Hz, 1703us
#define Note_E5 (659) // 659.26Hz, 1517us
#define Note_F5 (698) // 698.46Hz, 1432us
#define Note_G5 (784) // 784.00Hz, 1276us
#define Note_A5 (880) // 880.00Hz, 1136us
#define Note_B5 (988) // 987.77Hz, 1012us

#define Note_C6 (1047) // 1046.50Hz, 956us
#define Note_C6S (1108.73)
#define Note_D6 (1175) // 1174.66Hz, 851us
#define Note_E6 (1319) // 1318.51Hz, 758us
#define Note_F6 (1397) // 1396.91Hz, 716us
#define Note_G6 (1568) // 1567.98Hz, 638us
#define Note_A6 (1760) // 1760.00Hz, 568us
#define Note_B6 (1976) // 1975.53Hz, 506us

#define Note_C7 (2093) // 2093.00Hz, 478us
#define Note_D7 (2349) // 2349.32Hz, 426us
#define Note_E7 (2637) // 2637.02Hz, 379us
#define Note_F7 (2794) // 2793.83Hz, 358us
#define Note_G7 (3136) // 3135.96Hz, 319us
#define Note_A7 (3520) // 3520.00Hz, 284us
#define Note_B7 (3951) // 3951.07Hz, 253us

#define Note_b4 466
#define Note_00 (0)



#define EIGHTH_NOTE_DURATION (125)
#define FOURTH_NOTE_DURATION (250)
#define HALF_NOTE_DURATION (500)
#define WHOLE_NOTE_DURATION (1000)

#define MORSE_LONG 1865
#define MORSE_SHORT 932
#define MORSE_GAP 2637

#define QUINARY_0 Note_C5S
#define QUINARY_1 Note_F5
#define QUINARY_5 Note_C6S

#define QUINARY_TEST {\
    {QUINARY_0,200},\
    {Note_00,50},\
    {QUINARY_1,100},\
    {Note_00,100},\
    {QUINARY_5,150},\
    {Note_00,50}}

#define START {\
    {Note_00,WHOLE_NOTE_DURATION},\
    {Note_C5,EIGHTH_NOTE_DURATION},\
    {Note_C6,FOURTH_NOTE_DURATION},\
    {Note_00,WHOLE_NOTE_DURATION}}

#define END {\
    {Note_00,WHOLE_NOTE_DURATION},\
    {Note_C6,EIGHTH_NOTE_DURATION},\
    {Note_C5,FOURTH_NOTE_DURATION},\
    {Note_00,WHOLE_NOTE_DURATION}}

#define MORSE_TEST {\
    {Note_00,WHOLE_NOTE_DURATION},\
    {MORSE_SHORT,HALF_NOTE_DURATION},\
    {MORSE_LONG,WHOLE_NOTE_DURATION},\
    {Note_00,WHOLE_NOTE_DURATION}}

#define OVERPOWER {\
    {Note_00,WHOLE_NOTE_DURATION},\
    {Note_C6,FOURTH_NOTE_DURATION},\
    {Note_C6,FOURTH_NOTE_DURATION},\
    {Note_00,WHOLE_NOTE_DURATION}}

// #define XIAOMI {\
//     {Note_E5,EIGHTH_NOTE_DURATION},\
//     {Note_G5,EIGHTH_NOTE_DURATION},\
//     {Note_C6,EIGHTH_NOTE_DURATION},\
//     {Note_E5,EIGHTH_NOTE_DURATION},\
//     {Note_G5,EIGHTH_NOTE_DURATION},\
//     {Note_C6,EIGHTH_NOTE_DURATION},\
//     {Note_E5,EIGHTH_NOTE_DURATION},\
//     {Note_F5,EIGHTH_NOTE_DURATION},\
//     {Note_D5,EIGHTH_NOTE_DURATION},\
//     {Note_G5,EIGHTH_NOTE_DURATION},\
//     {Note_D6,EIGHTH_NOTE_DURATION},\
//     {Note_D5,EIGHTH_NOTE_DURATION},\
//     {Note_G5,EIGHTH_NOTE_DURATION},\
//     {Note_D6,EIGHTH_NOTE_DURATION},\
//     {Note_D5,EIGHTH_NOTE_DURATION},\
//     {Note_E5,EIGHTH_NOTE_DURATION},\
//     {Note_C5,EIGHTH_NOTE_DURATION},\
//     {Note_F5,EIGHTH_NOTE_DURATION},\
//     {Note_C6,EIGHTH_NOTE_DURATION},\
//     {Note_C5,EIGHTH_NOTE_DURATION},\
//     {Note_F5,EIGHTH_NOTE_DURATION},\
//     {Note_C6,EIGHTH_NOTE_DURATION},\
//     {Note_C5,EIGHTH_NOTE_DURATION},\
//     {Note_G5,EIGHTH_NOTE_DURATION},\
//     {Note_D5,EIGHTH_NOTE_DURATION},\
//     {Note_G5,EIGHTH_NOTE_DURATION},\
//     {Note_D6,EIGHTH_NOTE_DURATION},\
//     {Note_D5,EIGHTH_NOTE_DURATION},\
//     {Note_G5,EIGHTH_NOTE_DURATION},\
//     {Note_D6,EIGHTH_NOTE_DURATION},\
//     {Note_D5,EIGHTH_NOTE_DURATION},\
//     {Note_G5,EIGHTH_NOTE_DURATION},\
//     {Note_00,WHOLE_NOTE_DURATION}}
 
// #define FLYME {\
//     {Note_C5,EIGHTH_NOTE_DURATION},\
//     {Note_F4,EIGHTH_NOTE_DURATION},\
//     {Note_G4,EIGHTH_NOTE_DURATION},\
//     {Note_A4,EIGHTH_NOTE_DURATION},\
//     {Note_C5,EIGHTH_NOTE_DURATION},\
//     {Note_F5,EIGHTH_NOTE_DURATION},\
//     {Note_E5,EIGHTH_NOTE_DURATION},\
//     {Note_C5,EIGHTH_NOTE_DURATION},\
//     {Note_00,HALF_NOTE_DURATION},\
//     {Note_00,FOURTH_NOTE_DURATION},\
//     {Note_C5,EIGHTH_NOTE_DURATION},\
//     {Note_F4,EIGHTH_NOTE_DURATION},\
//     {Note_G4,EIGHTH_NOTE_DURATION},\
//     {Note_A4,EIGHTH_NOTE_DURATION},\
//     {Note_C5,EIGHTH_NOTE_DURATION},\
//     {Note_G5,EIGHTH_NOTE_DURATION},\
//     {Note_F5,EIGHTH_NOTE_DURATION},\
//     {Note_A5,EIGHTH_NOTE_DURATION},\
//     {Note_00,WHOLE_NOTE_DURATION}}

#define DDLC {\
    {Note_00,WHOLE_NOTE_DURATION},\
    {Note_00,WHOLE_NOTE_DURATION},\
    {Note_00,WHOLE_NOTE_DURATION},\
    {Note_00,WHOLE_NOTE_DURATION},\
    {Note_C5,EIGHTH_NOTE_DURATION},\
    {Note_C5,EIGHTH_NOTE_DURATION},\
    {Note_E5,EIGHTH_NOTE_DURATION},\
    {Note_C5,EIGHTH_NOTE_DURATION},\
    {Note_00,EIGHTH_NOTE_DURATION},\
    {Note_C5,EIGHTH_NOTE_DURATION},\
    {Note_F5,EIGHTH_NOTE_DURATION},\
    {Note_C5,EIGHTH_NOTE_DURATION},\
    {Note_00,EIGHTH_NOTE_DURATION},\
    {Note_D5,EIGHTH_NOTE_DURATION},\
    {Note_G5,115},\
    {Note_D6,EIGHTH_NOTE_DURATION},\
    {Note_C5,EIGHTH_NOTE_DURATION},\
    {Note_A4,EIGHTH_NOTE_DURATION},\
    {Note_C5,EIGHTH_NOTE_DURATION},\
    {Note_F5,EIGHTH_NOTE_DURATION},\
    {Note_00,EIGHTH_NOTE_DURATION},\
    {Note_C5,EIGHTH_NOTE_DURATION},\
    {Note_E5,EIGHTH_NOTE_DURATION},\
    {Note_C5,EIGHTH_NOTE_DURATION},\
    {Note_00,EIGHTH_NOTE_DURATION},\
    {Note_C5,EIGHTH_NOTE_DURATION},\
    {Note_F5,EIGHTH_NOTE_DURATION},\
    {Note_C5,EIGHTH_NOTE_DURATION},\
    {Note_00,EIGHTH_NOTE_DURATION},\
    {Note_D5,EIGHTH_NOTE_DURATION},\
    {Note_G5,115},\
    {Note_D6,EIGHTH_NOTE_DURATION},\
    {Note_C5,EIGHTH_NOTE_DURATION},\
    {Note_A4,EIGHTH_NOTE_DURATION},\
    {Note_C5,EIGHTH_NOTE_DURATION},\
    {Note_F5,EIGHTH_NOTE_DURATION},\
    {Note_00, WHOLE_NOTE_DURATION}}

// #define IED {\
//     {Note_C4, EIGHTH_NOTE_DURATION},\
//     {Note_C4, EIGHTH_NOTE_DURATION},\
//     {Note_C4, EIGHTH_NOTE_DURATION},\
//     {Note_C5, EIGHTH_NOTE_DURATION},\
//     {Note_G4, HALF_NOTE_DURATION},\
//     {Note_00, EIGHTH_NOTE_DURATION},\
//     {Note_C5, EIGHTH_NOTE_DURATION},\
//     {Note_C5, EIGHTH_NOTE_DURATION},\
//     {Note_C5, EIGHTH_NOTE_DURATION},\
//     {Note_C5, EIGHTH_NOTE_DURATION},\
//     {466, HALF_NOTE_DURATION},\
//     {Note_00, EIGHTH_NOTE_DURATION},\
//     {Note_C5, EIGHTH_NOTE_DURATION},\
//     {Note_C5, EIGHTH_NOTE_DURATION},\
//     {Note_C5, EIGHTH_NOTE_DURATION},\
//     {Note_C5, EIGHTH_NOTE_DURATION},\
//     {Note_F5, EIGHTH_NOTE_DURATION},\
//     {622, EIGHTH_NOTE_DURATION},\
//     {Note_D5, EIGHTH_NOTE_DURATION},\
//     {Note_C5, EIGHTH_NOTE_DURATION},\
//     {415, EIGHTH_NOTE_DURATION},\
//     {415, EIGHTH_NOTE_DURATION},\
//     {415, EIGHTH_NOTE_DURATION},\
//     {Note_C5, EIGHTH_NOTE_DURATION},\
//     {Note_F5, EIGHTH_NOTE_DURATION},\
//     {622, EIGHTH_NOTE_DURATION},\
//     {Note_D5, EIGHTH_NOTE_DURATION},\
//     {Note_C5, EIGHTH_NOTE_DURATION},\
//     {Note_00, WHOLE_NOTE_DURATION}}

// #define CKBK {\
//     {Note_G4, EIGHTH_NOTE_DURATION},\
//     {Note_A4, EIGHTH_NOTE_DURATION},\
//     {Note_G4, EIGHTH_NOTE_DURATION},\
//     {Note_D4, 350},\
//     {Note_00, EIGHTH_NOTE_DURATION},\
//     {Note_A4, FOURTH_NOTE_DURATION},\
//     {Note_G4, EIGHTH_NOTE_DURATION},\
//     {Note_A4, EIGHTH_NOTE_DURATION},\
//     {Note_G4, EIGHTH_NOTE_DURATION},\
//     {Note_D4, 350},\
//     {Note_00, EIGHTH_NOTE_DURATION},\
//     {466, FOURTH_NOTE_DURATION},\
//     {Note_A4, EIGHTH_NOTE_DURATION},\
//     {466, EIGHTH_NOTE_DURATION},\
//     {Note_A4, EIGHTH_NOTE_DURATION},\
//     {311, 350},\
//     {Note_00, EIGHTH_NOTE_DURATION},\
//     {466, FOURTH_NOTE_DURATION},\
//     {Note_A4, EIGHTH_NOTE_DURATION},\
//     {466, EIGHTH_NOTE_DURATION},\
//     {Note_A4, EIGHTH_NOTE_DURATION},\
//     {311, 350},\
//     {Note_00, EIGHTH_NOTE_DURATION},\
//     {Note_C5, FOURTH_NOTE_DURATION},\
//     {466, EIGHTH_NOTE_DURATION},\
//     {Note_C5, EIGHTH_NOTE_DURATION},\
//     {466, EIGHTH_NOTE_DURATION},\
//     {311, 350},\
//     {Note_00, EIGHTH_NOTE_DURATION},\
//     {Note_C5, FOURTH_NOTE_DURATION},\
//     {466, EIGHTH_NOTE_DURATION},\
//     {Note_C5, EIGHTH_NOTE_DURATION},\
//     {466, EIGHTH_NOTE_DURATION},\
//     {Note_A4, EIGHTH_NOTE_DURATION},\
//     {Note_G4, EIGHTH_NOTE_DURATION},\
//     {Note_A4, EIGHTH_NOTE_DURATION},\
//     {370, 450},\
//     {Note_00, WHOLE_NOTE_DURATION}}

// #define YDY {\
//     {Note_E5, HALF_NOTE_DURATION},\
//     {Note_00, EIGHTH_NOTE_DURATION},\
//     {Note_D5, EIGHTH_NOTE_DURATION},\
//     {Note_C5, EIGHTH_NOTE_DURATION},\
//     {Note_00, EIGHTH_NOTE_DURATION},\
//     {Note_A4, FOURTH_NOTE_DURATION},\
//     {Note_G4, FOURTH_NOTE_DURATION},\
//     {Note_A4, EIGHTH_NOTE_DURATION},\
//     {Note_G4, EIGHTH_NOTE_DURATION},\
//     {Note_E4, FOURTH_NOTE_DURATION},\
//     {Note_G4, FOURTH_NOTE_DURATION},\
//     {Note_C5, HALF_NOTE_DURATION},\
//     {Note_00, EIGHTH_NOTE_DURATION},\
//     {Note_C5, FOURTH_NOTE_DURATION},\
//     {Note_00, 80},\
//     {Note_C5, EIGHTH_NOTE_DURATION},\
//     {Note_00, 20},\
//     {Note_C5, EIGHTH_NOTE_DURATION},\
//     {Note_00, WHOLE_NOTE_DURATION}}

// #define RD1 {\
//     {Note_A3, EIGHTH_NOTE_DURATION},\
//     {Note_A3, EIGHTH_NOTE_DURATION},\
//     {Note_A3, EIGHTH_NOTE_DURATION},\
//     {Note_00, EIGHTH_NOTE_DURATION},\
//     {Note_B3, EIGHTH_NOTE_DURATION},\
//     {Note_B3, EIGHTH_NOTE_DURATION},\
//     {Note_B3, EIGHTH_NOTE_DURATION},\
//     {Note_00, EIGHTH_NOTE_DURATION},\
//     {Note_C4, FOURTH_NOTE_DURATION},\
//     {Note_00, 50},\
//     {Note_A3, FOURTH_NOTE_DURATION},\
//     {Note_00, FOURTH_NOTE_DURATION},\
//     {Note_A3, EIGHTH_NOTE_DURATION},\
//     {Note_A3, EIGHTH_NOTE_DURATION},\
//     {Note_A3, EIGHTH_NOTE_DURATION},\
//     {Note_00, EIGHTH_NOTE_DURATION},\
//     {Note_B3, EIGHTH_NOTE_DURATION},\
//     {Note_B3, EIGHTH_NOTE_DURATION},\
//     {Note_B3, EIGHTH_NOTE_DURATION},\
//     {Note_00, EIGHTH_NOTE_DURATION},\
//     {Note_E4, FOURTH_NOTE_DURATION},\
//     {Note_00, 50},\
//     {311, FOURTH_NOTE_DURATION},\
//     {Note_00, FOURTH_NOTE_DURATION},\
//     {Note_A3, EIGHTH_NOTE_DURATION},\
//     {Note_A3, EIGHTH_NOTE_DURATION},\
//     {Note_A3, EIGHTH_NOTE_DURATION},\
//     {Note_00, EIGHTH_NOTE_DURATION},\
//     {Note_B3, EIGHTH_NOTE_DURATION},\
//     {Note_B3, EIGHTH_NOTE_DURATION},\
//     {Note_B3, EIGHTH_NOTE_DURATION},\
//     {Note_00, EIGHTH_NOTE_DURATION},\
//     {Note_C4, FOURTH_NOTE_DURATION},\
//     {Note_00, 50},\
//     {Note_A3, FOURTH_NOTE_DURATION},\
//     {Note_E4, FOURTH_NOTE_DURATION},\
//     {311, FOURTH_NOTE_DURATION},\
//     {Note_E4, FOURTH_NOTE_DURATION},\
//     {311, FOURTH_NOTE_DURATION},\
//     {Note_B3, FOURTH_NOTE_DURATION},\
//     {233, FOURTH_NOTE_DURATION},\
//     {Note_B3, FOURTH_NOTE_DURATION},\
//     {233, FOURTH_NOTE_DURATION},\
//     {Note_00, WHOLE_NOTE_DURATION}}

// #define BESPRIDANNICA1 {\
//     {Note_D4, FOURTH_NOTE_DURATION},\
//     {Note_F4, FOURTH_NOTE_DURATION},\
//     {Note_D4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_E4, EIGHTH_NOTE_DURATION},\
//     {Note_D4, EIGHTH_NOTE_DURATION},\
//     {466, FOURTH_NOTE_DURATION},\
//     {Note_A4, FOURTH_NOTE_DURATION},\
//     {Note_G4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_E4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_D4, FOURTH_NOTE_DURATION},\
//     {Note_F4, FOURTH_NOTE_DURATION},\
//     {Note_D4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_E4, EIGHTH_NOTE_DURATION},\
//     {Note_D4, EIGHTH_NOTE_DURATION},\
//     {466, EIGHTH_NOTE_DURATION},\
//     {Note_A4, EIGHTH_NOTE_DURATION},\
//     {Note_G4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_E4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_E4, EIGHTH_NOTE_DURATION},\
//     {Note_00, EIGHTH_NOTE_DURATION}}

// #define BESPRIDANNICA2 {\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_00, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_E4, FOURTH_NOTE_DURATION},\
//     {Note_E4, FOURTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_A4, FOURTH_NOTE_DURATION},\
//     {Note_G4, FOURTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_00, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_E4, FOURTH_NOTE_DURATION},\
//     {Note_E4, FOURTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_A4, FOURTH_NOTE_DURATION},\
//     {Note_G4, FOURTH_NOTE_DURATION},\
//     {Note_00, 0}}

// #define BESPRIDANNICA3 {\
//     {Note_D4, EIGHTH_NOTE_DURATION},\
//     {Note_D4, EIGHTH_NOTE_DURATION},\
//     {Note_D4, EIGHTH_NOTE_DURATION},\
//     {Note_D4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_E4, EIGHTH_NOTE_DURATION},\
//     {Note_E4, EIGHTH_NOTE_DURATION},\
//     {Note_E4, EIGHTH_NOTE_DURATION},\
//     {Note_E4, EIGHTH_NOTE_DURATION},\
//     {Note_A4, EIGHTH_NOTE_DURATION},\
//     {Note_A4, EIGHTH_NOTE_DURATION},\
//     {Note_A4, EIGHTH_NOTE_DURATION},\
//     {Note_00, EIGHTH_NOTE_DURATION},\
//     {Note_D4, EIGHTH_NOTE_DURATION},\
//     {Note_D4, EIGHTH_NOTE_DURATION},\
//     {Note_D4, EIGHTH_NOTE_DURATION},\
//     {Note_D4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_E4, EIGHTH_NOTE_DURATION},\
//     {Note_E4, EIGHTH_NOTE_DURATION},\
//     {Note_E4, EIGHTH_NOTE_DURATION},\
//     {Note_E4, EIGHTH_NOTE_DURATION},\
//     {Note_C4, EIGHTH_NOTE_DURATION},\
//     {Note_C4, EIGHTH_NOTE_DURATION},\
//     {Note_C4, EIGHTH_NOTE_DURATION},\
//     {Note_00, EIGHTH_NOTE_DURATION},\
//     {Note_D4, EIGHTH_NOTE_DURATION},\
//     {Note_D4, EIGHTH_NOTE_DURATION},\
//     {Note_D4, EIGHTH_NOTE_DURATION},\
//     {Note_D4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_E4, EIGHTH_NOTE_DURATION},\
//     {Note_E4, EIGHTH_NOTE_DURATION},\
//     {Note_E4, EIGHTH_NOTE_DURATION},\
//     {Note_E4, EIGHTH_NOTE_DURATION},\
//     {Note_A4, EIGHTH_NOTE_DURATION},\
//     {Note_A4, EIGHTH_NOTE_DURATION},\
//     {Note_A4, EIGHTH_NOTE_DURATION},\
//     {Note_00, EIGHTH_NOTE_DURATION},\
//     {Note_D4, EIGHTH_NOTE_DURATION},\
//     {Note_D4, EIGHTH_NOTE_DURATION},\
//     {Note_D4, EIGHTH_NOTE_DURATION},\
//     {Note_D4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_A4, EIGHTH_NOTE_DURATION},\
//     {Note_A4, EIGHTH_NOTE_DURATION},\
//     {Note_G4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_E4, EIGHTH_NOTE_DURATION},\
//     {277, EIGHTH_NOTE_DURATION},\
//     {Note_00, FOURTH_NOTE_DURATION}}
  
// #define BESPRIDANNICA4 {\
//     {Note_D4, EIGHTH_NOTE_DURATION},\
//     {Note_A4, EIGHTH_NOTE_DURATION},\
//     {Note_G4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_E4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_E4, EIGHTH_NOTE_DURATION},\
//     {Note_D4, EIGHTH_NOTE_DURATION},\
//     {Note_D4, EIGHTH_NOTE_DURATION},\
//     {Note_A4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_E4, EIGHTH_NOTE_DURATION},\
//     {Note_D4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_A4, EIGHTH_NOTE_DURATION},\
//     {Note_00, EIGHTH_NOTE_DURATION},\
//     {Note_D4, EIGHTH_NOTE_DURATION},\
//     {Note_A4, EIGHTH_NOTE_DURATION},\
//     {Note_G4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_E4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_E4, EIGHTH_NOTE_DURATION},\
//     {Note_D4, EIGHTH_NOTE_DURATION},\
//     {Note_A4, EIGHTH_NOTE_DURATION},\
//     {Note_A4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_E4, EIGHTH_NOTE_DURATION},\
//     {Note_D4, EIGHTH_NOTE_DURATION},\
//     {Note_F4, EIGHTH_NOTE_DURATION},\
//     {Note_A4, EIGHTH_NOTE_DURATION},\
//     {Note_00, EIGHTH_NOTE_DURATION}}

#define HBD {\
    {Note_00,WHOLE_NOTE_DURATION},\
    {Note_00,WHOLE_NOTE_DURATION},\
    {Note_00,WHOLE_NOTE_DURATION},\
    {Note_00,WHOLE_NOTE_DURATION},\
    {Note_00,EIGHTH_NOTE_DURATION},\
    {Note_00,63},\
    {Note_00,FOURTH_NOTE_DURATION},\
    {Note_00,FOURTH_NOTE_DURATION},\
    {Note_00,FOURTH_NOTE_DURATION},\
    {Note_00,FOURTH_NOTE_DURATION},\
    {Note_00,FOURTH_NOTE_DURATION},\
    {Note_C5,EIGHTH_NOTE_DURATION},\
    {Note_C5,63},\
    {Note_D5,FOURTH_NOTE_DURATION},\
    {Note_C5,FOURTH_NOTE_DURATION},\
    {Note_G5,FOURTH_NOTE_DURATION},\
    {Note_F5,FOURTH_NOTE_DURATION},\
    {Note_00,FOURTH_NOTE_DURATION},\
    {Note_00,EIGHTH_NOTE_DURATION},\
    {Note_00,63},\
    {Note_00,FOURTH_NOTE_DURATION},\
    {Note_00,FOURTH_NOTE_DURATION},\
    {Note_00,FOURTH_NOTE_DURATION},\
    {Note_00,FOURTH_NOTE_DURATION},\
    {Note_00,FOURTH_NOTE_DURATION},\
    {932,EIGHTH_NOTE_DURATION},\
    {932,63},\
    {Note_A5,FOURTH_NOTE_DURATION},\
    {Note_F5,FOURTH_NOTE_DURATION},\
    {Note_G5,FOURTH_NOTE_DURATION},\
    {Note_F5,FOURTH_NOTE_DURATION},\
    {Note_00,FOURTH_NOTE_DURATION}}

#define INIT {\
    {Note_00,WHOLE_NOTE_DURATION},\
    {Note_00,WHOLE_NOTE_DURATION},\
    {Note_00,WHOLE_NOTE_DURATION},\
    {Note_00,WHOLE_NOTE_DURATION},\
    {Note_C5,EIGHTH_NOTE_DURATION},\
    {Note_C5,EIGHTH_NOTE_DURATION},\
    {Note_00,EIGHTH_NOTE_DURATION},\
    {Note_00,EIGHTH_NOTE_DURATION},\
    {Note_00,EIGHTH_NOTE_DURATION},\
    {Note_C5,EIGHTH_NOTE_DURATION},\
    {Note_F5,EIGHTH_NOTE_DURATION},\
    {Note_C5,EIGHTH_NOTE_DURATION},\
    {Note_00,EIGHTH_NOTE_DURATION}}  



typedef struct Buzzer_Note {
    uint16_t frequency;
    uint16_t period_us;
    uint16_t loudness;
} Buzzer_Note;

typedef struct Music_Data{
    uint16_t bpm;
    uint16_t psc;
    uint16_t pwm;
    uint16_t gap;
    uint16_t music_length;
    uint16_t music[70][2];
}Music_Data;

typedef enum
{
    STOP = 0,
    TOE,
    BATTERY,
    MUSIC,
    NUMBER_MODES,
} buzzer_mode;

typedef struct{
    uint16_t buzzer_mode;
    uint16_t buzzer_last_mode;
    uint32_t last_batteery_report_time;
    uint32_t battery_timeout;
    uint32_t last_toe_report_time;
    uint32_t toe_timeout;

}buzzer_control_t;

typedef struct
{
    uint8_t playing_flag;

    uint8_t battery_flag;
    uint8_t overpower_flag;
    uint8_t auto_toe_flag;
    uint8_t manual_toe_flag;
    uint8_t song_flag;
    uint8_t info_flag;
} buzzer_flag_t;

extern Music_Data rd1;

extern void buzzer_task(void const *argument);
extern const Buzzer_Note buzzer_notes[] ;
extern const uint16_t frequency[];
extern void buzzer_play(Music_Data music);
extern void play_toe_status(uint32_t toe_list);

extern void play_num_quinary(int number);

extern bool_t is_debug_key_pressed(void);
//void buzzer_toe_check(void);
#endif /* _BUZZER_TASK_H */

