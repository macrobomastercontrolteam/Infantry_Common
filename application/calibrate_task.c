...
static void RC_cmd_to_calibrate(void)
{
    ...
    else if (rc_action_flag == CHASSIS_FLAG && rc_cmd_time > RC_CMD_LONG_TIME)
    {
        rc_action_flag = 0;
        rc_cmd_time = 0;
        //chassis motor ID reset removed (bridge has no chassis motors)
        cali_buzzer_off();
    }
...
{
    // Gimbal removed on this bridge build: no physical gimbal to calibrate.
    // Keep the hook as a no-op so the calibration table/flash layout stays intact.
    (void)cali;
    if (cmd == CALI_FUNC_CMD_ON)
    {
        cali_buzzer_off();
        return 1;
    }
