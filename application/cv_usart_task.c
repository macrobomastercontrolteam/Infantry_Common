...
void CvCmder_toe_solve_lost_fun(void)
{
	...
	CvCmder_ChangeMode(CV_MODE_CHASSIS_ABS_ANGLE_BIT, 0);
}
	...
	switch (CvRxBuffer.tData.bMsgType)
	{
		...
		case MSG_MODE_CONTROL:
		{
			...
			if (fValid)
			{
				...
				CvCmder_ChangeMode(CV_MODE_CHASSIS_ABS_ANGLE_BIT, CvRxBuffer.tData.abPayload[0] & CV_MODE_CHASSIS_ABS_ANGLE_BIT);
			}
			else
			{
				CvCmder_ChangeMode(CV_MODE_SHOOT_BIT, 0);
