...
typedef struct
{
  ...
  pid_type_def steer_motor_angle_pid[4];       //steering motor angle PID.????????????pid
  chassis_steer_motor_t steer_motor_chassis[4];//chassis steering motor data.?????????????
  pid_type_def steer_motor_angle_pid[4];       //steering motor angle PID.????????????pid
#endif
  ...
  first_order_filter_type_t chassis_cmd_slow_set_wz;  //use first order filter to slow set-point.????????????????څ?
  first_order_filter_type_t chassis_cmd_slow_set_vx;  //use first order filter to slow set-point.????????????????څ?
  first_order_filter_type_t chassis_cmd_slow_set_vy;  //use first order filter to slow set-point.????????????????څ?
  first_order_filter_type_t chassis_cmd_slow_set_wz;  //use first order filter to slow set-point.????????????????څ?
